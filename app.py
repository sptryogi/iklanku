...
# --- HELPER KHUSUS TIKTOK ---
def clean_variasi_tiktok(text):
    if not isinstance(text, str) or pd.isna(text) or text == '':
        return ''
    # Ambil value di DEPAN koma (misal: "A5, Biru" -> "A5")
    if ',' in text:
        parts = text.split(',')
...
# --- LOGIKA PROSES DATA ---
def process_data(platform, toko, inputs):
    output = io.BytesIO()
    workbook = xlsxwriter.Workbook(output, {'in_memory': True})
    
    # --- FORMATTING (Sama untuk kedua platform) ---
    fmt_header = workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter', 'border': 1, 'bg_color': '#D9D9D9'})
    fmt_num = workbook.add_format({'align': 'center', 'valign': 'vcenter', 'border': 1})
    fmt_curr = workbook.add_format({'num_format': '#,##0', 'align': 'center', 'valign': 'vcenter', 'border': 1})
    fmt_date = workbook.add_format({'num_format': 'dd-mm-yyyy', 'align': 'left', 'valign': 'vcenter', 'italic': True})
    
    # Format Khusus Judul Tabel
    fmt_head_orange = workbook.add_format({'bold': True, 'align': 'center', 'border': 1, 'bg_color': '#FCE4D6', 'color': '#C65911'})
    fmt_head_green = workbook.add_format({'bold': True, 'align': 'center', 'border': 1, 'bg_color': '#E2EFDA', 'color': '#375623'})
    ...
    if platform == "TikTok":
        ...
        if 'ORDER STATUS' in df_orders.columns:
            df_orders = df_orders[df_orders['ORDER STATUS'] != 'Dibatalkan']
        
        # Bersihkan Angka (Quantity, Price, Discount)
        cols_num = ['QUANTITY', 'SKU UNIT ORIGINAL PRICE', 'SKU SELLER DISCOUNT']
        for c in cols_num:
            if c in df_orders.columns:
                df_orders[c] = pd.to_numeric(df_orders[c], errors='coerce').fillna(0)
                
        # Clean Variasi (Ambil depan koma)
        if 'VARIATION' in df_orders.columns:
            df_orders['VARIATION_CLEAN'] = df_orders['VARIATION'].apply(clean_variasi_tiktok)
        else:
            df_orders['VARIATION_CLEAN'] = "ALL"

        # 3. MERGE DENGAN CREATOR (Untuk Komisi Affiliate)
        # Orders: 'ORDER ID' <-> Creator: 'ID PESANAN'
        if 'ID PESANAN' in df_creator.columns and 'PERKIRAAN PEMBAYARAN KOMISI STANDAR' in df_creator.columns:
            # Pastikan ID jadi string biar match
            df_orders['ORDER ID_STR'] = df_orders['ORDER ID'].astype(str)
            df_creator['ID PESANAN_STR'] = df_creator['ID PESANAN'].astype(str)
            
            # Ambil hanya kolom penting dari creator
            aff_lookup = df_creator[['ID PESANAN_STR', 'PERKIRAAN PEMBAYARAN KOMISI STANDAR']].copy()
            aff_lookup['PERKIRAAN PEMBAYARAN KOMISI STANDAR'] = pd.to_numeric(aff_lookup['PERKIRAAN PEMBAYARAN KOMISI STANDAR'], errors='coerce').fillna(0)
            
            # Merge
            df_orders = df_orders.merge(aff_lookup, left_on='ORDER ID_STR', right_on='ID PESANAN_STR', how='left')
            df_orders.rename(columns={'PERKIRAAN PEMBAYARAN KOMISI STANDAR': 'KOMISI_AFFILIATE'}, inplace=True)
            df_orders['KOMISI_AFFILIATE'] = df_orders['KOMISI_AFFILIATE'].fillna(0)
        else:
            df_orders['KOMISI_AFFILIATE'] = 0

        # 4. HITUNG OMZET (Logic: (Price - (Disc/Qty)) * Qty)
        # Note: Logic user: [Original] - ([Seller Disc] / [Qty]) -> Ini harga per unit diskon
        # Lalu dikali [Qty] lagi.
        def calc_omzet_tiktok(row):
            qty = row['QUANTITY']
            if qty <= 0: return 0
            price = row['SKU UNIT ORIGINAL PRICE']
            disc_total = row['SKU SELLER DISCOUNT'] # Asumsi disc di file adalah total discount per line
            
            # Rumus User: (Price - (Disc/Qty)) * Qty
            # Sederhananya: (Price * Qty) - Disc
            # Kita ikuti rumus user exact steps:
            price_after_disc_per_unit = price - (disc_total / qty)
            return price_after_disc_per_unit * qty

        df_orders['OMZET_PENJUALAN'] = df_orders.apply(calc_omzet_tiktok, axis=1)

        # 5. SIAPKAN DATA TABEL 5 (RINCIAN SELURUH PESANAN)
        # Group by PRODUCT NAME (Nama Produk jangan double)
        # Kita perlu Variasi juga. Ambil variasi pertama yang muncul atau gabungkan.
        # User bilang: "Nama Produk... jangan sampai ada yang double berarti semua yang sama itu dijumlahin"
        # Tapi ada kolom Variasi di output. Saya akan ambil variasi 'terbanyak' atau 'pertama'.
        
        grp_order = df_orders.groupby('PRODUCT NAME').agg({
            'VARIATION_CLEAN': 'first', # Ambil salah satu variasi
            'QUANTITY': 'sum',
            'OMZET_PENJUALAN': 'sum',
            'KOMISI_AFFILIATE': 'sum'
        }).reset_index()

        # 6. SIAPKAN DATA TABEL 2 (RINCIAN BIAYA IKLAN)
        # Dari file Product Data
        # Columns: Nama Produk, Biaya, ROI
        t2_data = []
        if not df_product.empty:
            ...
            # Rename kolom df_product sesuai kebutuhan
            df_prod_clean = df_product.rename(columns=col_map)
            
            # Ambil kolom yang ada saja
            avail_cols = [c for c in ['Nama Produk', 'Biaya Iklan', 'ROI'] if c in df_prod_clean.columns]
            t2_data = df_prod_clean[avail_cols].copy()
            
            # Pastikan numeric
            if 'Biaya Iklan' in t2_data.columns:
                t2_data['Biaya Iklan'] = pd.to_numeric(t2_data['Biaya Iklan'], errors='coerce').fillna(0)
            if 'ROI' in t2_data.columns:
                t2_data['ROI'] = pd.to_numeric(t2_data['ROI'], errors='coerce').fillna(0)
        
        # ==========================================
        # WRITING TO EXCEL (TIKTOK LAYOUT)
        # ==========================================
        
        report_date = datetime.now().strftime("%A, %d-%m-%Y")
        ws.write(0, 0, "LAPORAN IKLAN", fmt_header)
        ws.write(1, 0, report_date, fmt_date)
        
        current_row = 3
        
        # --- TABEL 2: RINCIAN BIAYA IKLAN ---
        ws.merge_range(current_row, 0, current_row, 2, 'RINCIAN BIAYA IKLAN', fmt_head_orange)
        current_row += 1
        
        # Header T2
        t2_headers = ['Nama produk yang diiklankan', 'Biaya Iklan', 'ROI']
        for i, h in enumerate(t2_headers):
            ws.write(current_row, i, h, fmt_head_orange)
        current_row += 1
        
        # Isi T2
        total_biaya_iklan = 0
        if isinstance(t2_data, pd.DataFrame) and not t2_data.empty:
            for _, row in t2_data.iterrows():
                ws.write(current_row, 0, row.get('Nama Produk', '-'), fmt_num)
                val_biaya = row.get('Biaya Iklan', 0)
                ws.write(current_row, 1, val_biaya, fmt_curr)
                ws.write(current_row, 2, row.get('ROI', 0), fmt_num)
                total_biaya_iklan += val_biaya
                current_row += 1
        
        # Total T2
        ws.write(current_row, 0, "TOTAL", fmt_head_orange)
        ws.write(current_row, 1, total_biaya_iklan, fmt_head_orange)
        ws.write(current_row, 2, "", fmt_head_orange)
        
        current_row += 2 # Spasi antar tabel
        
        # --- TABEL 5: RINCIAN SELURUH PESANAN ---
        t5_start_col = 0
        # Header Utama T5 (Header + Total Qty Disampingnya)
        total_qty_all = grp_order['QUANTITY'].sum()
        ws.write(current_row, t5_start_col, 'RINCIAN SELURUH PESANAN', fmt_head_green)
        ws.write(current_row, t5_start_col + 1, total_qty_all, fmt_head_green)
        ws.merge_range(current_row, t5_start_col + 2, current_row, t5_start_col + 4, "", fmt_head_green)
        current_row += 1
        
        # Header Kolom T5
        t5_cols = ['Nama Produk', 'Variasi', 'Jumlah Eksemplar', 'Omzet Penjualan', 'Total Komisi Affiliate']
        for i, c in enumerate(t5_cols):
            ws.write(current_row, t5_start_col + i, c, fmt_head_green)
        current_row += 1
        ...
        for _, row in grp_order.iterrows():
            ws.write(current_row, 0, row['PRODUCT NAME'], fmt_num) # Text Wrap otomatis di Excel biasanya perlu set align
            ws.write(current_row, 1, row['VARIATION_CLEAN'], fmt_num)
            ws.write(current_row, 2, row['QUANTITY'], fmt_num)
            ws.write(current_row, 3, row['OMZET_PENJUALAN'], fmt_curr)
            ws.write(current_row, 4, row['KOMISI_AFFILIATE'], fmt_curr)
            
            total_omzet += row['OMZET_PENJUALAN']
            total_affiliate += row['KOMISI_AFFILIATE']
            current_row += 1
            
        # Total T5
        ws.write(current_row, 0, "TOTAL", fmt_head_green)
        ws.write(current_row, 1, "", fmt_head_green)
        ws.write(current_row, 2, total_qty_all, fmt_head_green)
        ws.write(current_row, 3, total_omzet, fmt_head_green)
        ws.write(current_row, 4, total_affiliate, fmt_head_green)
        
        current_row += 2 # Spasi
        
        # --- TABEL 6: TOTAL PENJUALAN (SUMMARY) ---
        # Format: Label | Value
        # Total Penjualan (Omzet)
        # Total Biaya Iklan (T2)
        # Total Komisi Affiliate (T5)
        # ROI = Total Penjualan / (Biaya Iklan + Komisi Affiliate)
        
        cost_total = total_biaya_iklan + total_affiliate
        roi_final = (total_omzet / cost_total) if cost_total > 0 else 0
        
        summary_items = [
            ("Total Penjualan", total_omzet, fmt_curr),
            ("Total Biaya Iklan", total_biaya_iklan, fmt_curr),
            ("Total Komisi Affiliate", total_affiliate, fmt_curr),
            ("ROI", roi_final, fmt_num)
        ]
        
        for label, val, fmt in summary_items:
            ws.write(current_row, 0, label, fmt_head_orange)
            ws.write(current_row, 1, val, fmt)
            current_row += 1

        # Adjust column width (Simple)
        ws.set_column(0, 0, 40) # Nama Produk lebar
        ws.set_column(1, 4, 20) 

    # ==========================================
    # LOGIKA SHOPEE (EXISTING)
    # ==========================================
    elif platform == "Shopee":
        # Ambil file dari inputs dictionary
        f_order = inputs['order']
        f_iklan = inputs['iklan']
        f_seller = inputs['seller']
        ...
        # 1. LOAD DATA
        df_order = pd.read_excel(f_order, dtype={'Total Harga Produk': str, 'Jumlah': str, 'Harga Satuan': str})
        df_iklan = pd.read_csv(f_iklan, skiprows=7)
        
        # Cek apakah file_seller ada isinya (Optional)
        if f_seller is not None:
            df_seller = pd.read_csv(f_seller, dtype={'Pengeluaran(Rp)': str})
        else:
...
# 1. Pilih Platform
platform = st.radio("Pilih Platform Marketplace:", ["Shopee", "TikTok"], horizontal=True)

# Input Toko
toko = st.selectbox("Pilih Toko:", ["Human Store", "Pacific Bookstore", "DAMA.ID STORE"])

# 3. Input File (Conditional based on Platform)
files_ready = False
inputs = {}
...
if platform == "Shopee":
    ...
    with col1:
        inputs['order'] = st.file_uploader("Upload 'Order-all' (xlsx)", type=['xlsx'], key="s_order")
    with col2:
        inputs['iklan'] = st.file_uploader("Upload 'Iklan Keseluruhan' (csv)", type=['csv'], key="s_iklan")
    with col3:
        inputs['seller'] = st.file_uploader("Upload 'Seller conversion' (csv) - Opsional", type=['csv'], key="s_seller")
    
    if inputs['order'] and inputs['iklan']:
        files_ready = True

elif platform == "TikTok":
    st.info("Upload File Khusus TikTok (Format .xlsx)")
    col1, col2, col3 = st.columns(3)
    with col1:
        inputs['all_orders'] = st.file_uploader("Upload 'Semua Pesanan' (xlsx)", type=['xlsx'], key="t_order")
    with col2:
        inputs['product_data'] = st.file_uploader("Upload 'Product Data' (xlsx)", type=['xlsx'], key="t_prod")
    with col3:
        inputs['creator_order'] = st.file_uploader("Upload 'Creator Order-all' (xlsx)", type=['xlsx'], key="t_creator")
        
    if inputs['all_orders'] and inputs['product_data'] and inputs['creator_order']:
        files_ready = True
...
if st.button("Mulai Proses", type="primary"):
    if files_ready:
        with st.spinner('Sedang memproses data... Tunggu sebentar ya...'):
            try:
                # Panggil process_data dengan argumen platform
                excel_file = process_data(platform, toko, inputs)
                ...
                if excel_file:
                    ...
                    st.download_button(
                        ...
                        data=excel_file,
                        file_name=f"LAPORAN_{platform.upper()}_{toko.replace(' ', '_').upper()}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
            except Exception as e:
                st.error(f"Terjadi Kesalahan: {e}")
                st.error("Pastikan format file benar dan kolom yang dibutuhkan tersedia.")
    else:
        st.warning("Mohon upload semua file yang dibutuhkan.")
