...
def clean_variasi_tiktok(text, product_name=""):
    ...
    is_paket_wakaf = "Alquran Paket Wakaf Murah 50 pcs Al Aqeel | Alquran 18 Baris" in str(product_name)
...
def process_tiktok_data(toko, file_order, file_product, file_creator):
    ...
    def get_eksemplar_tiktok(row):
        ...
        if "Alquran Paket Wakaf Murah 50 pcs Al Aqeel | Alquran 18 Baris" in str(row['PRODUCT NAME']):
...
def process_data(toko, file_order, file_iklan, file_seller):
    ...
    if "Pacific Bookstore" in toko:
        ...
        b_grosir = get_biaya_regex(r"Paket.*Hemat.*Grosir.*Wakaf", case_sensitive=False)
